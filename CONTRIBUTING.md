...
# 构建并冒烟某个示例（basic / layout / events / phase5 / form-controls / virtual-list / inspector）
...
- `example`：示例改动。
- 子系统名：`theme` / `animation` / `state` / `layout` / `list` / `inspector` / `event` / `diff` / `native` / `render` / `widget` / `docs` / `ci` / `scripts`。
...
- 关联 issue：`Closes #123`。
- 触发 CI：`go test ./...` + `go vet ./...` + 构建冒烟（basic/events/phase5/form-controls/virtual-list/inspector）。
